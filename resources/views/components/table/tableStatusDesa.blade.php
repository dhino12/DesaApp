<table class="table-fixed w-full border border-gray-300 divide-y divide-slate-100 dark:divide-slate-700 data-table capitalize" border="2">
    <thead class="bg-slate-200 dark:bg-slate-700">
        <tr>
            {{-- Loop through columns and exclude unwanted columns --}}
            @foreach ($columns as $column)
                @if (str_contains($column, 'id') || str_contains($column, 'created') || str_contains($column, 'updated'))
