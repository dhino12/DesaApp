...
@section('container_content')
    ...
    <div class="wizard card">
        <div x-data="app()" x-cloak>
            ...
            @endif
            <form action="/dashboard/form-indikator-desa/{{ request()->segment(3) }}" method="post">
            @method("PUT")
            @csrf
                <div class="max-w-5xl mx-auto px-4 py-10">
                    <div x-show.transition="step === 'complete'">
                        <div class="bg-white rounded-lg p-10 flex items-center shadow justify-center">
                            <div>
                                ...
                                <h2 class="text-2xl mb-4 text-gray-800 text-center font-bold">Ubah Data Success {{ $tab }} <br />Periode {{ $data['created_at'] }} | {{ $provinces[0]->name_provinsi }}</h2>

                                <div class="text-gray-600 mb-8">
                                    Terima Kasih. Kami akan mengirimkan email terkait laporan perubahan sebelumnya.
                                </div>
                    ...
                    <div x-show.transition="step != 'complete'">	
                        ...
                        <div class="border-b-2 py-4">
                            <div class="uppercase tracking-wide text-xs font-bold text-gray-500 mb-1 leading-tight" x-text="`Step: ${step} of 1`"></div>
                            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                                <div class="flex-1">
                                    <div x-show="step === 1">
                                        <div class="text-lg font-bold text-gray-700 leading-tight">{{ $tab }}</div>
                                    </div>
                                </div>
                            ...
                            <div x-show.transition.in="step === 1" id="indikator-1">
                                <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
                                    ...
                                    @foreach (array_keys($data) as $column)
                                        ...
                                        @else
                                        <div class="input-area col-span-1 flex flex-col justify-between rounded-lg">
                                            <label for="username" class="form-label text-lg font-semibold"> {{ str_replace('_', ' ', $column) }} <strong class="text-red-500">*</strong></label>
                                            <div class="text-gray-600 mt-2 mb-4 text-xs">> Desa dalam wilayah Kabupaten/Kota</div>
                                            <input id="indikator" type="number" 
                                                class="form-control h-10 rounded-md border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                                                placeholder="Jumlah Desa" name="{{ str_replace('_', ' ', $column) }}" 
                                                required value="{{ $data[$column] }}">
                                            @error(str_replace('_', ' ', $column))
                                                <span class="badge bg-danger-500 text-white capitalize inline-flex items-center mt-2">
                                                    <iconify-icon class="ltr:mr-1 rtl:ml-1" icon="heroicons:exclaimation-triangle"></iconify-icon> 
                                                    {{ str_replace('_', ' ', $column) }} harus diisi
                                                </span>
                                            @enderror
                                        </div>
                                        @endif
                        ...
                        <button 
                            @click="if (confirm('Yakin mengubah data?')) { step = 'complete'; } else {event.preventDefault()}"
                            x-show="step === 1"
